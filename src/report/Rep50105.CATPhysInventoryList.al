report 50105 "CAT Phys. Inventory List"
{
    DefaultLayout = Word;
    WordLayout = './layouts/Carbon Engineering Physical Inventory List.docx';
    ...
    dataset
    {
        dataitem(PageLoop; "Integer")
        {
            ...
            //>>CAT
            column(CATUserID; USERID)
            {
            }
            column(CATQtyCalcToPrint_Caption; CATQtyCalcToPrint_Caption)
            {
            }
            //<<CAT            
            dataitem("Item Journal Batch"; "Item Journal Batch")
            {
                ...
                dataitem("Item Journal Line"; "Item Journal Line")
                {
                    ...
                    //>>CAT
                    column(CATQtyCalcToPrint; CATQtyCalcToPrint)
                    {
                    }

                    dataitem(CATExtendedTextHeader; "Extended Text Header")
                    {
                        DataItemLinkReference = "Item Journal Line";
                        DataItemLink = "No." = FIELD("Item No.");
                        DataItemTableView = SORTING("Table Name", "No.", "Language Code", "Text No.") ORDER(Ascending) WHERE("Table Name" = CONST(Item), "CAT Print on Phys. Inv. List" = CONST(true));
                        column(CATExtText_Description; Description)
                        {
                        }
                        dataitem(CATExtendedTextLine; "Extended Text Line")
                        {
                            DataItemLinkReference = CATExtendedTextHeader;
                            DataItemLink = "Table Name" = FIELD("Table Name"), "No." = FIELD("No."), "Text No." = FIELD("Text No."), "Language Code" = FIELD("Language Code");
                            DataItemTableView = SORTING("Table Name", "No.", "Language Code", "Text No.", "Line No.") ORDER(Ascending);
                            column(CATExtTextLine_LineNo; CATExtendedTextLine."Line No.")
                            {
                            }
                            column(CATExtTextLine_Text; CATExtendedTextLine."Text")
                            {
                            }
                        }
                    }
                    ...
                    begin
                        ...
                        if ShowQtyCalculated then
                            CATQtyCalcToPrint := FORMAT("Qty. (Calculated)")
                        else
                            CATQtyCalcToPrint := '';
                        //<<CAT                                            
                    end;
            ...
            begin
                if ShowQtyCalculated then
                    CATQtyCalcToPrint_Caption := 'Qty. (Calculated)'
                else
                    CATQtyCalcToPrint_Caption := '';
    ...
    var
        ...
        //>>CAT
        CATQtyCalcToPrint: Text;
        CATQtyCalcToPrint_Caption: Text;
    //<<CAT
