report 50106 "CAT ExportElecPayments - Word"
{
    // CAT.001 - 2023-01-19 CL Copied from 11383 "ExportElecPayments - Word". The orig std report is copied to the end of this file.
    //    - See codeunit 50100 subscriber to OnAfterSubstituteReport where the standard report is replaced.
    //    - this is to fix issue where the standard report is using LCY and FCY inconsistently. 
    //    - we are creating a new report instead of a report extension. Previously, we created a repext but it looks like Microsoft is changing the way the report is working to handle
    //      the currency issue and the report is being modified in new versions. Their latest change broke our previous repext. It still doesn't seem to fixed yet, so they will likely change the
    //      base report some more. In the meantime, we created this new report, which can be deleted when Microsoft finishes fixing their report.

    //--CAT.001RDLCLayout = './Local/ExportElecPaymentsWord.rdlc';
    //--CAT.001WordLayout = './ExportElecPaymentsWord.docx';
    //>>CAT.001
    RDLCLayout = './layouts/ExportElecPaymentsWord.rdlc';
    WordLayout = './layouts/ExportElecPaymentsWord.docx';
    //<<CAT.001
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem("Vendor Ledger Entry"; "Vendor Ledger Entry")
                    {
                        ...
                        column(Remaining_Amt___LCY___Control36; -"Remaining Amount") //++CAT.001
                        {
                        }
                    ...
                    dataitem(CustomerInfo; "Integer")
                    {
                        ...
                        //--CAT.001column(Cust_Remaining_Amt_LCY; -CustLedgEntry."Remaining Amt. (LCY)")
                        column(Cust_Remaining_Amt_LCY; -CustLedgEntry."Remaining Amount") //++CAT.001
                        {
                        }
                    ...
                    {
                        DataItemTableView = SORTING(Number) ORDER(Ascending) WHERE(Number = CONST(1));
                        //--CAT.001column(Vend_Remaining_Amt_LCY; -VendLedgEntry."Remaining Amt. (LCY)")
                        column(Vend_Remaining_Amt_LCY; -VendLedgEntry."Remaining Amount") //++CAT.001
                        {
                        }
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    group(OutputOptions)
                    {
                        ...
                        //>>CAT.001 start delete - comment out because it causes error. It is not visible anyway.
    ...
    begin
        ...
        if GenJnlLine."Applies-to Doc. No." <> '' then begin
            ...
            //--CAT.001VendLedgEntry.CalcFields("Remaining Amt. (LCY)", Amount);
            VendLedgEntry.CalcFields("Remaining Amt. (LCY)", "Remaining Amount", Amount); //++CAT.001
    ...
    begin
        ...
        if GenJnlLine."Applies-to Doc. No." <> '' then begin
            ...
            //--CAT.001CustLedgEntry.CalcFields("Remaining Amt. (LCY)", Amount);
            CustLedgEntry.CalcFields("Remaining Amt. (LCY)", "Remaining Amount", Amount); //++CAT.001
