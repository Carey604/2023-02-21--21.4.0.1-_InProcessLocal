page 50102 "CAT Purchase Requisition"
{
    // CAT.001 2020-12-09 - copied from page 49 and modified. The original code is copied to end of this file for comparison purposes
    // - simplified purchase quote page
    // - add action and functionality to change vendor
    // - there is a eventsubscriber in cod50100 that sets the card page to this page 50102 when
    //   the header."CAT Requisition" value is true.
    // CAT.002 2021-01-22 CL - change promoted on Vendor Order No. and Purchaser Code. Revert to default of Additional. 
    ...
    SourceTableView = WHERE("Document Type" = FILTER(Quote), "CAT Requisition" = CONST(true));
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                //>>CAT.001 
                field("CAT Assigned User Approver ID"; Rec."CAT Assigned User Approver ID")
                {
                    Visible = false;
                    ApplicationArea = All;
                    Importance = Standard;
                    ToolTip = 'Specifies the Approver ID for the user who is specified in Assigned User ID.';
                }
                field("CAT Approving Manager ID"; Rec."CAT Approving Manager ID")
                {
                    ApplicationArea = All;
                    Importance = Standard;
                    ToolTip = 'Specifies the approving manager''s ID.';
                }
                field("CAT Requisition Priority"; Rec."CAT Requisition Priority")
                {
                    ApplicationArea = All;
                    Importance = Standard;
                    ToolTip = 'Specifies the priority for the document.';
                }
                field("CAT Valid SDS"; Rec."CAT Valid SDS")
                {
                    ApplicationArea = All;
                    Importance = Standard;
                    Visible = false;
                    ToolTip = 'If SDS''s are applicable for the requisition, specifies whether there is a valid SDS for the items found on the purchase lines.';
                }
                field("CAT Submitted SDS"; Rec."CAT Submitted SDS")
                {
                    ApplicationArea = All;
                    Importance = Standard;
                    Visible = false;
                    ToolTip = 'Specifies whether the SDS''s have been submitted for approval.';
                }
                //>>CAT.003
                ...
                field("CAT Shortcut Dimension 2 Code"; Rec."Shortcut Dimension 2 Code") //copied from Invoice Details group
                {
                    ApplicationArea = Dimensions;
                    Importance = Standard;
            ...
            //--CAT.001part(PurchLines; "Purchase Quote Subform")
            part(PurchLines; "CAT Purchase Requisition Subf.")
            {
                ApplicationArea = Suite;
                Editable = "Buy-from Vendor No." <> '';
    ...
    actions
    {
        area(navigation)
        {
            group("&Quote")
            {
                ...
                Image = Quote;
                //>>CAT.001
                action(CATChangeBuyFromVendor)
                {
                    ApplicationArea = Suite;
                    Caption = 'Change &Buy-From Vendor';
                    Enabled = Rec."Buy-from Vendor No." <> '';
                    Image = Vendor;
                    Promoted = true;
                    PromotedCategory = Category7;
                    ToolTip = 'Change the buy-from vendor on the requisition without updating certain other information such as pricing and line recreation.';

                    trigger OnAction()
                    begin
                        Rec.CATUpdateBuyFromVendorNo();
                        CurrPage.Update();
                    end;
                }
        ...
        area(processing)
        {
            ...
            //>>CAT.001
            //there is a new subscriber to Cod93_OnBeforePurchQuoteToOrder in Cod50100 that checks and disallows make order if it is a requisition
            group("CAT Convert to Quote")
            {
                Caption = 'Convert Requisition to Quote';
                Image = MakeOrder;
                action(CATConvertToQuote)
                {
                    ApplicationArea = Suite;
                    Caption = '&Convert Requisition to Quote';
                    Image = MakeOrder;
                    ...
                    PromotedCategory = Process;
                    PromotedIsBig = true;
                    ToolTip = 'Convert the Requisition quote to a Quote.';

                    trigger OnAction()
                    var
                    begin
                        Rec.CATChangeRequistitionToQuote();
                        //CurrPage.Update(true);
                        CurrPage.Update(false);//17.0.0.5++ 
                    end;
                }
            }
            //<<CAT.001
    ...
    var
    //VerifySDSMsg: label 'On Purchase Requisition %1 Please ensure that there is a valid SDS when purchasing chemicals.';
    begin
        ...
        CalculateCurrentShippingAndPayToOption;
        //>>CAT.001
        "CAT Requisition" := true;
        //<<CAT.001
        //>>CAT.002
        // CATNotification.Message := StrSubstNo(VerifySDSMsg, rec."No.");
        // CATNotification.Scope := NotificationScope::LocalScope;
        // CatNotification.Send();
        //message(StrSubstNo(VerifySDSMsg, rec."No."));
        //<<CAT.002
    end;
    ...
    var
        VerifySDSMsg: label 'Please ensure that there is a valid SDS when purchasing chemicals.';
    begin
        "CAT Requisition" := true;
        CATNotification.Message := VerifySDSMsg; //No. is blank at this point so cannot pass it
        CATNotification.Scope := NotificationScope::LocalScope;
        CATNotification.Send();
    end;
    //<<CAT.002
    ...
    var
        ...
        CATNotification: Notification; //++CAT.002
        [InDataSet]
        StatusStyleTxt: Text;
...
//         area(factboxes)
//         {
//             part("Attached Documents"; Rec."Document Attachment Factbox")
//             {
//                 ApplicationArea = All;
//                 Caption = 'Attachments';
//                 SubPageLink = "Table ID" = CONST(38),
