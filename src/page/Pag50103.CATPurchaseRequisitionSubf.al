page 50103 "CAT Purchase Requisition Subf."
{
    // CAT.001 2020-12-09 CL - copied from page 97 Purchase Quote Subform. Original code is copied to end of this file.
    //      - simplified subform
    // CAT.002 2021-08-20 CL - add fiix field 
    // CAT.003 2021-08-31 CL - unhide location code, add bin code
    // CAT.004 2021-09-23 CL - add expected receipt date field 10
    // CAT.005 2021-09-27 CL - remove expected receipt date and replace with Order date.
    // CAT.006 2021-09-28 CL - undo CAT.005, change to requested receipt date
    AutoSplitKey = true;
    Caption = 'Lines';
    DelayedInsert = true;
    LinksAllowed = false;
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                field(Type; Rec.Type)
                {
                    //--CAT.001ApplicationArea = Advanced;
                ...
                //CAT.001 start delete - because AutoCompleteOption is onprem only
                ...
                field("Bin Code"; Rec."Bin Code")
                {
                    ...
                    Editable = NOT IsBlankNumber;
                    Enabled = NOT IsBlankNumber;
                    ToolTip = 'Specifies a bin code for the location where you want to receive the items.';
                }
                //<<CAT.003
    ...
    actions
    {
        area(processing)
        {
            ...
            group("&Line")
            {
                ...
                Image = Line;
                //>>CAT.001
                action(CATChangeGLAccNo)
                {
                    ApplicationArea = All;
                    Caption = 'Change &GL Account No.';
                    Image = Change;
                    ToolTip = 'Change the G/l Account No.';
                    Enabled = Rec.Type = Rec.Type::"G/L Account";

                    trigger OnAction()
                    begin
                        Rec.CATUpdateGLAccNo();
                    end;
                }
                //<<CAT.001
                group("Item Availability by")
                {
                    ...
                    Enabled = Rec.Type = Rec.Type::Item;
                    //--CAT.001 action(Event)
                    action(CATEvent) //++CAT.001 rename to CATEvent otherwise when save, it generates 150+ errors for some reason, even though there is no error before save
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Event';
                        Image = "Event";
    ...
    begin
        ...
        //--CAT.001TempOptionLookupBuffer.FillBuffer(TempOptionLookupBuffer."Lookup Type"::Purchases); //because FillBuffer is OnPrem only
    ...
    begin
        ...
        //>>CAT.001 - suppress showing dims 6-8
        DimVisible6 := false;
        DimVisible7 := false;
        DimVisible8 := false;
        //<<CAT.001
